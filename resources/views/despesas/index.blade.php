        ...
        <div class="flex items-center justify-between bg-white p-4 rounded-lg shadow">
            ...
            <div class="flex space-x-2">
                <a href="?month={{ $currentMonth }}&year={{ $currentYear }}" 
                   class="px-4 py-2 rounded-lg border-2 {{ request('recurrence') === null ? 'bg-primary text-white border-primary' : 'border-gray-300 text-gray-600 hover:border-primary hover:text-priamry' }} transition-colors">
                    Tudo
                </a>
                <a href="?month={{ $currentMonth }}&year={{ $currentYear }}&recurrence=mensal" 
                   class="px-4 py-2 rounded-lg border-2 {{ request('recurrence') === 'mensal' ? 'bg-primary text-white border-primary' : 'border-gray-300 text-gray-600 hover:border-primary hover:text-priamry' }} transition-colors">
                    Fixas
                </a>
                <a href="?month={{ $currentMonth }}&year={{ $currentYear }}&recurrence=única" 
                   class="px-4 py-2 rounded-lg border-2 {{ request('recurrence') === 'única' ? 'bg-primary text-white border-primary' : 'border-gray-300 text-gray-600 hover:border-primary hover:text-priamry' }} transition-colors">
                    Variáveis
                </a>
            </div>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($expenses as $expense)
                        <tr class="hover:bg-gray-50 transition-colors group cursor-pointer" onclick="window.location='{{ route('despesas.show', $expense) }}'">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-gray-900 group-hover:text-primary transition-colors">{{ \Carbon\Carbon::parse($expense->due_date)->format('d/m/Y') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-gray-900 group-hover:text-primary transition-colors">{{ $expense->expense_name }}</td>
                            <td class="px-6 py-4 text-gray-900 group-hover:text-primary transition-colors">{{ $expense->expense_description }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-gray-900 group-hover:text-primary transition-colors">{{ $expense->expense_category }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-gray-900 font-medium group-hover:text-primary transition-colors">R$ {{ number_format($expense->expense_value, 2, ',', '.') }}</td>
